...
def render_kpi_card(title, value, is_neon=False):
    ...
    st.markdown(
        ...
        <div class="glass-card">
            ...
            <div class="kpi-value {neon_class}">{value}</div>
        </div>
        """,
        unsafe_allow_html=True
    )
...
with st.sidebar:
    ...
    nav_mode = st.radio(
        "OPERATING LAYER", 
        ["🌐 Global Overview", "📈 Market Dynamics", "⚡ Risk & Diagnostics", "🔮 Predictive Forecasting"], 
    ...
    st.markdown("<hr style='border-color: rgba(102,252,241,0.2);'>", unsafe_allow_html=True)
    st.markdown("<p style='color: #45a29e; font-size:0.85rem; font-weight:600; margin-bottom:10px;'>FLEET FILTER</p>", unsafe_allow_html=True)
...
if nav_mode == "🌐 Global Overview":
    ...
    timeline_df = filtered_df.groupby('Date')['Total_Withdrawals'].sum().reset_index()
    fig_timeline = px.line(timeline_df, x='Date', y='Total_Withdrawals', line_shape='spline', color_discrete_sequence=['#66fcf1'])
    ...
    if len(selected_metrics) < 2:
        ...
        with col_dyn2:
            ...
            fig_dist = px.box(filtered_df, y=selected_metrics[0], color="Location_Type", color_discrete_sequence=['#66fcf1', '#45a29e', '#1f2833', '#c5c6c7'])
            fig_dist.update_layout(template=base_template, margin=dict(t=30, b=10, l=10, r=10), showlegend=False, height=450)
    ...
    if len(selected_metrics) < 2:
        ...
        with st.spinner("Compiling Neural Models..."):
            ...
            time.sleep(0.4)
            st.toast("Neural Engine Recalibrated Successfully", icon="✅")
    ...
    st.markdown('<div class="glass-card">', unsafe_allow_html=True)
    st.markdown("### 📅 Withdrawal Velocity by Day of Week")
    day_order = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
    daily_stats = filtered_df.groupby('Day_of_Week')['Total_Withdrawals'].mean().reindex(day_order).reset_index()
    ...
    with col_for1:
        st.markdown('<div class="glass-card">', unsafe_allow_html=True)
        st.markdown("### 🔄 7-Day Rolling Trajectory")
        fig_rolling = px.line(filtered_df.groupby('Date')['Rolling_Mean_Withdrawals'].mean().reset_index(), x='Date', y='Rolling_Mean_Withdrawals', line_shape='spline', color_discrete_sequence=['#45a29e'])
    ...
    with col_for2:
        st.markdown('<div class="glass-card">', unsafe_allow_html=True)
        st.markdown("### 🔀 Day-over-Day Volatility (%)")
        fig_vol = px.histogram(filtered_df, x='Daily_Change_Pct', nbins=50, color_discrete_sequence=['#66fcf1'])
